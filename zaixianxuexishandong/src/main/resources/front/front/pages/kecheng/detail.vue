	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
				...
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text
							style="font-weight:bold;font-size:50rpx">{{detail.kechengNewMoney}}</text><text
							style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.kechengOldMoney!=null">原价￥<text
									style="text-decoration:line-through">{{detail.kechengOldMoney}}</text></text>
						</text>
					</view>
				</view>
			...
			<view style="text-align: center;padding: 20upx;box-sizing: border-box;background: #FFFFFF;"
				:style="{margin:'0 60rpx 0 60rpx',boxShadow:'0 0 4rpx 0 #006633 insta'}" v-if="orderList > 0">
			...
			<view v-if="orderList == 0" class="bottom-content cu-bar bg-white tabbar border shop">
		...
		async onShow(options) {
			...
			if(this.id){
				let params = {
					...
					limit: 9999,
					kechengId: this.id,
					yonghuId: res.data.id
				}
				let orderList = await this.$api.list('kechengOrder', params);
				this.orderList = orderList.data.list.length
			}
		...
		methods: {
			...
			onSubmit() {
				uni.setStorageSync('orderGoods', [{
					...
					kechengName: this.detail.kechengName,
					kechengPhoto: this.swiperList[0],
					buyNumber: 1,
					kechengNewMoney: this.detail.kechengNewMoney,
				}])
				this.$utils.jump('../kechengOrder/confirm');
			},
