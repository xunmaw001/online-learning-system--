    ...
    <view class="content">
        <form>
            ...
            <view v-for="(item,index) in orderGoods " v-bind:key="index" class="cu-form-group">
                <image class="avator" :src="item.kechengPhoto" mode=""></image>
                <view class="title" style="width: 75%;">
                    <view style="margin-top: -50rpx;">{{item.kechengName}}</view>
                    <view >
                        x{{item.buyNumber}}
                        <text style="margin-left: 30upx;color: red;">￥{{item.kechengNewMoney}}</text>
                    </view>
                </view>
            </view>
            <view @tap="onAddressTap" class="cu-form-group">
                ...
                <view>
                    <text v-if="kechengOrderPaymentTypes == 1">原价：￥{{(maxNewMouey).toFixed(2)}}</text>
                    <view v-if="kechengOrderPaymentTypes == 1"></view>
                    <text v-if="kechengOrderPaymentTypes == 1">折扣价：￥{{(maxNewMouey * zhekou).toFixed(2)}}</text>
                    <text v-if="kechengOrderPaymentTypes == 2">{{(maxNewMouey).toFixed(2)}}积分</text>
                </view>
            </view>
        </form>
    ...
    export default {
        data() {
            return {
                ...
                orderGoods: [],//展示数据
                maxNewMouey: 0,//总价格
                kechengOrderPaymentTypes:1,//是什么支付类型
                zhi:[
                    {
                        id:1,
                        val:"余额"
        ...
        async onLoad(options) {
            ...
            if(this.orderGoods.length>0){
                for (let i = 0; i < this.orderGoods.length; i++) {
                    this.maxNewMouey = this.maxNewMouey + parseFloat(this.orderGoods[i].kechengNewMoney) * this.orderGoods[i].buyNumber
                }
            }
            ...
            async onSubmitTap() {
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
							 let data = {
								 kechengId: _this.orderGoods[0].kechengId,
								 yonghuId: _this.user.id,
							 }
							await _this.$api.add('kechengOrder',data);
                            _this.$utils.jump('/pages/kechengOrder/list');
                        }
                    }
                });
            },
