...
//                            kechengOrderEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R add(@RequestBody KechengOrderEntity kechengOrder, HttpServletRequest request){
        ...
        YonghuEntity yonghuEntity = yonghuService.selectById(kechengOrder.getYonghuId());
        if(yonghuEntity != null && yonghuEntity.getNewMoney()<kechengEntity.getKechengNewMoney()){
            return R.error("余额不足请充值");
        }
        yonghuEntity.setNewMoney(yonghuEntity.getNewMoney()-kechengEntity.getKechengNewMoney());
